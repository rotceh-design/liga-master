...
import { useAuth } from '../context/AuthContext'; // Usamos tu interruptor de Comisionado/Público
...
const MATCHES_DB = [
  {
    ...
    home: {
      name: "PATRIOTAS", full: "PATRIOTAS FC", abbr: "PAT", color: "#000000", // Negro para el local
      players: [
    ...
    away: {
      name: "RIVALS", full: "RIVALS FC", abbr: "RIV", color: "#52525B", // Gris zinc para el visita
      players: [
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTE 1: SELECTOR DE PARTIDO (Estilo Limpio)
// ─────────────────────────────────────────────────────────────────────────────
function MatchSelector({ onEnter, isAdmin, onToggleAdmin }) {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex flex-col animate-fade-in">
      {/* Top Bar */}
      <div className="flex items-center justify-between px-6 h-14 bg-white border-b border-zinc-200">
        <div className="flex items-center gap-4">
          <span className="font-black text-xl tracking-tighter">LIGA PRO</span>
          <span className="text-zinc-300">|</span>
          <span className="font-bold text-xs tracking-widest text-zinc-500 uppercase">Selección de Partido</span>
        </div>
        <button onClick={onToggleAdmin} className={`px-4 py-1.5 font-bold text-xs uppercase tracking-widest border transition-colors ${isAdmin ? 'border-black bg-black text-white' : 'border-zinc-300 text-zinc-500 hover:border-black hover:text-black'}`}>
          {isAdmin ? 'Modo: Comisionado' : 'Modo: Público'}
        </button>
      </div>

      {/* Header */}
      <div className="p-8 md:p-12 border-b border-zinc-200 bg-white">
        <h1 className="font-black text-4xl md:text-6xl tracking-tighter uppercase mb-2">Jornada 14</h1>
        <p className="font-bold text-sm tracking-widest text-zinc-400 uppercase">Elige el encuentro a transmitir</p>
      </div>

      {/* Grid de Partidos */}
      <div className="flex-1 p-6 md:p-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 align-start">
        {MATCHES_DB.map((m) => (
          <div 
            key={m.id} 
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <div className="flex flex-col items-center flex-1">
                  ...
                  <div className="font-black text-lg tracking-tight uppercase">{m.home.name}</div>
                </div>
                <div className="flex flex-col items-center px-4">
                  <span className="font-black text-2xl text-zinc-300">VS</span>
                </div>
                <div className="flex flex-col items-center flex-1">
                  <div className="w-12 h-12 bg-white border-2 border-zinc-200 text-black rounded-full flex items-center justify-center font-black text-xl mb-2">{m.away.abbr[0]}</div>
                  <div className="font-black text-lg tracking-tight text-zinc-500 uppercase">{m.away.name}</div>
                </div>
              </div>
      ...
      {/* Bottom Bar */}
      <div className="p-6 bg-white border-t border-zinc-200 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <button 
          ...
          className="px-8 py-4 bg-black text-white font-black text-sm uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed hover:bg-zinc-800 transition-colors w-full md:w-auto"
        >
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTE 2: CENTRO EN VIVO (Scoreboard + Panel + Timeline)
// ─────────────────────────────────────────────────────────────────────────────
function LiveCenter({ match, isAdmin, onToggleAdmin, onBack }) {
  ...
  // Subcomponente de barra de progreso para estadísticas
  ...
  return (
    <div className="flex flex-col h-screen bg-zinc-50 overflow-hidden animate-fade-in">
      ...
      {/* ═══ TOP BAR ═══ */}
      <div className="flex items-center justify-between px-6 h-14 bg-white border-b border-zinc-200 shrink-0">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="text-xs font-bold uppercase tracking-widest text-zinc-500 hover:text-black transition-colors">
            ← Volver
          </button>
          <span className="text-zinc-300">|</span>
          <span className="font-black text-lg tracking-tighter">LIGA PRO</span>
          <span className="text-zinc-300">|</span>
          <span className="font-bold text-xs tracking-widest text-zinc-900 uppercase">{match.home.name} vs {match.away.name}</span>
        </div>
        <div className="flex items-center gap-6">
          {isLive && (
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-red-600 animate-pulse" />
              <span className="font-bold text-[10px] tracking-widest text-red-600 uppercase">En Vivo</span>
            </div>
          )}
          <button onClick={onToggleAdmin} className={`px-4 py-1.5 font-bold text-[10px] uppercase tracking-widest border transition-colors ${isAdmin ? 'border-black bg-black text-white' : 'border-zinc-300 text-zinc-500 hover:border-black hover:text-black'}`}>
            {isAdmin ? 'Modo: Comisionado' : 'Modo: Público'}
          </button>
        </div>
      </div>

      {/* ═══ MARCADOR (Estilo Clean) ═══ */}
      <div className="bg-white border-b border-zinc-200 shrink-0 pb-6">
        <div className="flex justify-center items-center gap-4 pt-4 pb-2">
          <span className="font-bold text-[10px] tracking-widest text-zinc-400 uppercase">{match.jornada} · {match.estadio}</span>
        </div>

        <div className="flex justify-center items-center px-6">
          {/* Local */}
          <div className="flex flex-col items-end flex-1 pr-8">
            <div className="w-12 h-12 bg-black text-white rounded-full flex items-center justify-center font-black text-xl mb-2">{match.home.abbr[0]}</div>
            <span className="font-black text-2xl tracking-tighter uppercase">{match.home.name}</span>
          </div>

          {/* Puntaje y Reloj */}
          <div className="flex items-center gap-6">
            <span className="font-black text-6xl md:text-8xl tracking-tighter tabular-nums">{homeScore}</span>
            <div className="flex flex-col items-center px-4">
              <div className="font-mono font-black text-2xl bg-zinc-100 px-4 py-1 border border-zinc-200 mb-1">{fmtTime(seconds)}</div>
              <span className="font-bold text-[10px] text-zinc-400 tracking-widest uppercase">{half===1?"1er Tiempo":half===2?"2do Tiempo":"Finalizado"}</span>
            </div>
            <span className="font-black text-6xl md:text-8xl tracking-tighter tabular-nums text-zinc-400">{awayScore}</span>
          </div>

          {/* Visita */}
          <div className="flex flex-col items-start flex-1 pl-8">
            <div className="w-12 h-12 bg-white border-2 border-zinc-200 text-black rounded-full flex items-center justify-center font-black text-xl mb-2">{match.away.abbr[0]}</div>
            <span className="font-black text-2xl tracking-tighter text-zinc-500 uppercase">{match.away.name}</span>
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* ────── IZQUIERDA (Controles o Vista Público) ────── */}
        <div className="flex-1 flex flex-col border-r border-zinc-200 bg-white overflow-hidden">
          
          {isAdmin ? (
            <>
              {/* BARRA DE RELOJ (Solo Admin) */}
              <div className="flex items-stretch shrink-0 border-b border-zinc-200 bg-zinc-50">
                <button onClick={() => { running ? setRunning(false) : (setRunning(true), setIsLive(true)) }} className={`px-6 font-black text-xs uppercase tracking-widest border-r border-zinc-200 transition-colors ${running ? 'bg-white text-black' : 'bg-black text-white hover:bg-zinc-800'}`}>
                  {running ? '⏸ Pausar' : '▶ Iniciar'}
                </button>
                <button onClick={() => { setRunning(false); setSeconds(0); }} className="px-6 font-bold text-xs uppercase tracking-widest text-zinc-500 hover:text-black border-r border-zinc-200 transition-colors">
                  ↺ Reset
                </button>
                {[1, 2].map(h => (
                  <button key={h} onClick={() => setHalf(h)} className={`px-6 font-black text-xs uppercase tracking-widest border-r border-zinc-200 transition-colors ${half === h ? 'border-b-2 border-b-black text-black' : 'text-zinc-400 hover:text-black'}`}>
                    {h}T
                  </button>
                ))}
                <button onClick={() => setIsLive(v => !v)} className={`ml-auto px-6 font-bold text-[10px] uppercase tracking-widest transition-colors ${isLive ? 'text-red-600 bg-red-50' : 'text-zinc-400 hover:text-black'}`}>
                  {isLive ? '● Transmisión Activa' : '○ Activar Transmisión'}
                </button>
              </div>
              ...
              <div className="flex shrink-0 border-b border-zinc-200">
                {TABS.map(t => (
                  <button key={t} onClick={() => setTab(t)} className={`px-6 py-3 font-black text-[11px] uppercase tracking-widest transition-colors relative ${tab === t ? 'text-black' : 'text-zinc-400 hover:text-zinc-700'}`}>
                    {t}
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-8">
                ...
                {/* TAB: EVENTOS (Formulario Principal) */}
                {tab === "Eventos" && (
                  <div className="max-w-3xl animate-fade-in">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-8">
                      {EV_TYPES.map(t => (
                        <button key={t.id} onClick={() => setEvType(t.id)} className={`py-3 px-2 border flex flex-col items-center justify-center gap-1 transition-all ${evType === t.id ? 'border-black bg-zinc-50 shadow-sm scale-[1.02]' : 'border-zinc-200 hover:border-zinc-400'}`}>
                          <span className="text-xl">{t.emoji}</span>
                          <span className={`font-black text-[9px] uppercase tracking-widest ${evType === t.id ? 'text-black' : 'text-zinc-500'}`}>{t.label}</span>
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                      {["home", "away"].map(side => (
                        <button key={side} onClick={() => setEvTeam(side)} className={`py-4 border-2 font-black text-sm uppercase tracking-widest transition-all ${evTeam === side ? 'border-black bg-zinc-50' : 'border-zinc-200 text-zinc-400 hover:border-zinc-400'}`}>
                          {match[side].name}
                        </button>
                      ))}
                    </div>

                    {/* 3. Detalles del Jugador */}
                    {evType === "sub" ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                        ...
                        <div>
                          ...
                          <input type="number" placeholder={`${Math.floor(seconds/60)}`} value={evMin} onChange={e => setEvMin(e.target.value)} className="w-full p-3 border border-zinc-300 bg-white outline-none focus:border-black font-mono font-black text-center" />
                        </div>
                      </div>
                    )}
                ...
                {tab === "Plantillas" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-fade-in">
                    {["home", "away"].map(side => (
                      <div key={side}>
                        <h3 className="font-black text-lg uppercase tracking-widest mb-4 border-b border-black pb-2">{match[side].name}</h3>
                ...
                {tab === "Estadísticas" && (
                  <div className="max-w-2xl mx-auto animate-fade-in">
                    <div className="flex justify-between items-center mb-8 border-b border-black pb-4">
                      <span className="font-black text-xl uppercase tracking-tighter">{match.home.name}</span>
                      <span className="font-bold text-[10px] tracking-widest text-zinc-400 uppercase">Comparativa de Juego</span>
                      <span className="font-black text-xl uppercase tracking-tighter text-zinc-400">{match.away.name}</span>
                    </div>
            ...
            /* VISTA PÚBLICO (Hincha) */
            <div className="flex-1 flex flex-col items-center justify-center p-8 bg-zinc-100">
              <div className="w-24 h-24 rounded-full border-4 border-black flex items-center justify-center text-4xl mb-6 relative bg-white shadow-xl">
                ⚽
                {isLive && <div className="absolute inset-[-8px] rounded-full border-2 border-red-500 animate-pulse" />}
              </div>
              <h2 className="font-black text-3xl uppercase tracking-tighter mb-2">{isLive ? "Partido en Curso" : "Esperando Inicio"}</h2>
              <p className="font-bold text-sm tracking-widest text-zinc-500 uppercase mb-8">Sigue los detalles en la bitácora lateral</p>
              
              {isLive && (
                <div className="px-6 py-2 bg-black text-white font-black text-xs uppercase tracking-widest animate-pulse flex items-center gap-3">
        ...
        <div className="w-80 md:w-96 flex flex-col bg-zinc-50 border-l border-zinc-200 shrink-0">
          <div className="h-14 flex items-center justify-between px-6 border-b border-zinc-200 bg-white shrink-0">
            <span className="font-black text-sm uppercase tracking-widest">Play-by-Play</span>
            {events.length > 0 && <span className="px-2 py-0.5 bg-black text-white font-black text-[10px]">{events.length}</span>}
          </div>

          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {events.length === 0 && (
              <p className="text-center font-bold text-xs uppercase tracking-widest text-zinc-400 mt-10">Sin eventos aún</p>
            )}
            {[...events].reverse().map(ev => {
              const def = EV_TYPES.find(t => t.id === ev.type) || EV_TYPES[0];
              return (
                <div key={ev.id} className="bg-white border border-zinc-200 p-4 animate-slide-up hover:border-black transition-colors">
                  ...
                  <div className="font-black text-sm uppercase leading-tight">{ev.player}</div>
                  {ev.note && <div className="font-medium text-xs text-zinc-500 mt-2 border-t border-zinc-100 pt-2">{ev.note}</div>}
                </div>
              );
            })}
            <div ref={feedEndRef} />
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTE RAÍZ (Orquestador)
// ─────────────────────────────────────────────────────────────────────────────
export default function LiveMatch() {
  const { isAdmin, toggleRole } = useAuth(); 
  const [screen, setScreen] = useState("selector"); // "selector" | "live"
